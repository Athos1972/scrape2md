...
(() => {
  ...
  for (const sel of selectors) {
    for (const el of document.querySelectorAll(sel)) {
      if (!el || clicked.has(el)) continue;
      try { el.click(); clicked.add(el); } catch (e) {}
    }
  }
...
class CrawlEngine:
    ...
    def _fetch_with_crawl4ai(
                    ...
                    crawl_args.update(
                        {
                            ...
                            "wait_for": self._wait_for or crawl_args.get("wait_for"),
                        }
                    )

                    js_chunks: list[str] = []
                    if self._enable_menu_clicks:
                        js_chunks.append(_GENERIC_MENU_JS)
                    if self._js_code_before_wait:
                        js_chunks.append(self._js_code_before_wait)
                    if js_chunks:
                        crawl_args["js_code_before_wait"] = "\n".join(js_chunks)
                    if self._js_code:
                        crawl_args["js_code"] = self._js_code
        ...
        title = getattr(result, "title", None) or _extract_title(html)
